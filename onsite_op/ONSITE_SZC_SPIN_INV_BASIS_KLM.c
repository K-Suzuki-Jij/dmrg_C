...
void ONSITE_SZC_SPIN_INV_BASIS_KLM(int spin_loc, CRS1 *M, double coeef) {
   ...
   if (spin_loc <= 0) {
      printf("Error in ONSITE_SZC_SPIN_INV_BASIS_KLM\n");
      printf("2spin=%d\n",spin_loc);
      exit(1);
   }
   ...
   int row_c,col_c,row_s,col_s,elem_num,parity_row,parity_col;
   double val;
      ...
      for (row_s = 0; row_s < dim_spin; row_s++) {
         for (col_c = 0; col_c < dim_charge; col_c++) {
            for (col_s = 0; col_s < dim_spin; col_s++) {
               parity_row = row_c/dim_parity;
               parity_col = col_c/dim_parity;
