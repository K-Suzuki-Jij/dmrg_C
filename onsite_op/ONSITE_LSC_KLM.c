...
void ONSITE_LSC_KLM(int num, int spin_loc, CRS1 *M, double coeef) {
   ...
   if (spin_loc <= 0 || num < 0 || (spin_loc + 1)*(spin_loc + 1)*2 <= num) {
      printf("ONSITE_LSC_KLM\n");
   ...
   int c_op       = num%2 + 1;
   int ls_row     = (num/2)/dim_spin;
   int ls_col     = (num/2)%dim_spin;
      ...
      for (row_s = 0; row_s < dim_spin; row_s++) {
         for (col_c = 0; col_c < dim_charge; col_c++) {
            for (col_s = 0; col_s < dim_spin; col_s++) {
               ...
               if (row_s == ls_row && col_s == ls_col) {
                  if (c_op == 1) {
                     if ((col_c == 3 && row_c == 2) || (col_c == 1 && row_c == 0)) {
                        val = coeef;
                     }
                     else {
                        val = 0.0;
