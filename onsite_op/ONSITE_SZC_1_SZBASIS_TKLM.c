...
void ONSITE_SZC_1_SZBASIS_TKLM(int spin_loc, CRS1 *M, double coeef) {
   ...
   if (spin_loc <= 0) {
      printf("Error in ONSITE_SZC_1_SZBASIS_TKLM\n");
      printf("2spin=%d\n",spin_loc);
      exit(1);
   }
            ...
            for (col_c_1 = 0; col_c_1 < dim_charge; col_c_1++) {
               for (col_c_2 = 0; col_c_2 < dim_charge; col_c_2++) {
                  for (col_s = 0; col_s < dim_spin; col_s++) {
                     a = row_c_1;
                     b = col_c_1;
                     val  = (spin_charge*0.5 + 1.0 - b)*DELTA_FUNCTION(a, b);
                     if (row_c_1 == 0 || row_c_1 == 3 || col_c_1 == 0 || col_c_1 == 3) {
                        val = 0.0;
                     }
                     
                     val = val*coeef;
                     
                     if (fabs(val) > zero && row_s == col_s && row_c_2 == col_c_2) {
                        M->Val[elem_num] = val;
