...
void CONJUGATE_GRADIENT(BOX_CG *Box){
   ...
   if (Box->M->row_dim != Box->M->col_dim) {
      printf("Error in CONJUGATE_GRADIENT\n");
   ...
   MATRIX_VECTOR_PRODUCT(Box->M, Box->Out_Vec, RRR, Box->p_threads);
...
#pragma omp parallel for num_threads (Box->p_threads)
   ...
   for (iter = 1; iter < Box->max_step; iter++) {
      ...
      MATRIX_VECTOR_PRODUCT(Box->M, PPP, YYY, Box->p_threads);
...
#pragma omp parallel for num_threads (Box->p_threads)
   ...
   fprintf(file, "Error in CONJUGATE_GRADIENT\n");
