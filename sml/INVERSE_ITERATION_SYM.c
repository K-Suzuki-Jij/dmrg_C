...
void INVERSE_ITERATION_SYM(BOX_II *Box_II) {
   ...
   if (Box_II->M->row_dim != Box_II->M->col_dim) {
      printf("Error in INVERSE_ITERATION_SYM\n");
   ...
   for (iter = 0; iter <= Box_II->ii_max_step; iter++) {
      ...
      error = CONVERGE_CHECK_SYM(Box_II->M, Box_II->Eig_Vec, Box_II->ii_diag_add, Box_II->p_threads);
      ...
      if (strcmp(Box_II->Type,"CG") == 0) {
         CONJUGATE_GRADIENT_SYM(Box_CG);
      }
      else if(strcmp(Box_II->Type,"MR") == 0){
         MINIMUM_RESIDUAL_SYM(Box_CG);
      }
      else {
         printf("Error in INVERSE_ITERATION_SYM\n");
   ...
   fprintf(file, "Error in INVERSE_ITERATION_SYM\n");
