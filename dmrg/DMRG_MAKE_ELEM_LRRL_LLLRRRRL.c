...
void DMRG_MAKE_ELEM_LRRL_LLLRRRRL(DMRG_BASIS_ONSITE *Basis, DMRG_A_BASIS *A_Basis, int *Inv_LLLRRRRL, CRS1 *M_LR, CRS1 *M_RL, double coeef, int *Ele_RR, int *Ele_LR, int *elem_num, char Type[], char Sign_Flag[]) {
   ...
   if (strcmp(Sign_Flag, "Yes") == 0) {
      if (strcmp(Type, "LR_RL") == 0) {
         ...
         printf("Error in DMRG_MAKE_ELEM_LRRL_LLLRRRRL\n");
         exit(1);
      }
   }
   else {
   ...
   for (i = M_LR->Row[LR]; i < M_LR->Row[LR + 1]; i++) {
      ...
      for (j = M_RL->Row[RL]; j < M_RL->Row[RL + 1]; j++) {
         col_RL    = M_RL->Col[j];
         val_RL    = M_RL->Val[j];
         inv_sup   = (long)LL*dim_onsite*dim_RR*dim_onsite + (long)col_LR*dim_RR*dim_onsite + RR*dim_onsite + col_RL;
