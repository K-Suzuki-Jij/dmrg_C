...
DMRG_SYSTEM_INFO *DMRG_GET_SYSTEM_INFO_Q2(int *Q_Number1, int *Q_Number2, DMRG_BASIS *Dmrg_Basis, double *Vec, int max_dim, int p_threads, DMRG_TIME *Dmrg_Time) {
   ...
   DMRG_DENSITY_MATRIX_INFO *Info = DMRG_GET_DENSITY_MATRIX_INFO_Q2(Q_Number1, Q_Number2, dim_LLLR);
...
#pragma omp parallel for private (dim) num_threads (p_threads)
   ...
   //Check Point 1
   if (count != dim_LLLR) {
      printf("Error in DMRG_MAKE_TRANSFORMATION_MATRIX_Q2 at Check Point 1\n");
      exit(1);
   }
   ...
   //Check Point 2
   if (dim_renorm <= 0) {
      printf("Error in DMRG_MAKE_TRANSFORMATION_MATRIX_Q2 at Check Point 2\n");
      exit(1);
   }
   ...
   if (elem_num != Dmrg_System->Trans_Matrix->max_val) {
      printf("Error in DMRG_MAKE_TRANSFORMATION_MATRIX_Q2 at Check Point 3\n");
      exit(1);
   }
