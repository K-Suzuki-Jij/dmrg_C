...
DMRG_SYSTEM_INFO *DMRG_GET_SYSTEM_INFO_Q3(int *Q_Number1, int *Q_Number2, int *Q_Number3, DMRG_BASIS *Dmrg_Basis, double *Vec, int max_dim, int p_threads, DMRG_TIME *Dmrg_Time) {
   ...
   DMRG_DENSITY_MATRIX_INFO *Info = DMRG_GET_DENSITY_MATRIX_INFO_Q3(Q_Number1, Q_Number2, Q_Number3, dim_LLLR);
...
#pragma omp parallel for private (dim) num_threads (p_threads)
   ...
   int *Q_Number1_Sorted = GET_ARRAY_INT1(dim_LLLR);
   int *Q_Number2_Sorted = GET_ARRAY_INT1(dim_LLLR);
   int *Q_Number3_Sorted = GET_ARRAY_INT1(dim_LLLR);
   ...
   for (block = 0; block < Info->block_num; block++) {
      for (iter1 = 0; iter1 < Info->Dim_Block[block]; iter1++) {
         ...
         Q_Number1_Sorted[count] = Info->Q_Number1_Block[block];
         Q_Number2_Sorted[count] = Info->Q_Number2_Block[block];
         Q_Number3_Sorted[count] = Info->Q_Number3_Block[block];
         count++;
      }
   }
   ...
   //Check Point 1
   if (count != dim_LLLR) {
      printf("Error in DMRG_MAKE_TRANSFORMATION_MATRIX_Q3 at Check Point 1\n");
      exit(1);
   }
   ...
   for (iter1 = 0; iter1 < dim_LLLR - 1; iter1++) {
      ...
      for (iter2 = 0; iter2 < dim_LLLR - 1 - iter1; iter2++) {
         ...
         if (val1 < val2) {
            ...
            SWAP_INT(&Q_Number1_Sorted[iter2], &Q_Number1_Sorted[iter2 + 1]);
            SWAP_INT(&Q_Number2_Sorted[iter2], &Q_Number2_Sorted[iter2 + 1]);
            SWAP_INT(&Q_Number3_Sorted[iter2], &Q_Number3_Sorted[iter2 + 1]);
            count = 1;
         }
      }
      if (count == 0) {
   ...
   //Check Point 2
   if (dim_renorm <= 0) {
      printf("Error in DMRG_MAKE_TRANSFORMATION_MATRIX_Q3 at Check Point 2\n");
      exit(1);
   }
   ...
   for (iter1 = 0; iter1 < dim_renorm - 1; iter1++) {
      ...
      for (iter2 = 0; iter2 < dim_renorm - 1 - iter1; iter2++) {
         if (Q_Number3_Sorted[iter2] > Q_Number3_Sorted[iter2 + 1]) {
            SWAP_INT(&Block_Sorted[iter2]   , &Block_Sorted[iter2 + 1]);
            SWAP_INT(&Basis_Sorted[iter2]   , &Basis_Sorted[iter2 + 1]);
            SWAP_INT(&Q_Number1_Sorted[iter2], &Q_Number1_Sorted[iter2 + 1]);
            SWAP_INT(&Q_Number2_Sorted[iter2], &Q_Number2_Sorted[iter2 + 1]);
   ...
   for (iter1 = 0; iter1 < dim_renorm - 1; iter1++) {
      count = 0;
      for (iter2 = 0; iter2 < dim_renorm - 1 - iter1; iter2++) {
         if (Q_Number2_Sorted[iter2] > Q_Number2_Sorted[iter2 + 1]) {
            SWAP_INT(&Block_Sorted[iter2]   , &Block_Sorted[iter2 + 1]);
            SWAP_INT(&Basis_Sorted[iter2]   , &Basis_Sorted[iter2 + 1]);
            SWAP_INT(&Q_Number1_Sorted[iter2], &Q_Number1_Sorted[iter2 + 1]);
            SWAP_INT(&Q_Number2_Sorted[iter2], &Q_Number2_Sorted[iter2 + 1]);
   ...
   for (iter1 = 0; iter1 < dim_renorm - 1; iter1++) {
      count = 0;
      for (iter2 = 0; iter2 < dim_renorm - 1 - iter1; iter2++) {
         if (Q_Number1_Sorted[iter2] > Q_Number1_Sorted[iter2 + 1]) {
            SWAP_INT(&Block_Sorted[iter2]   , &Block_Sorted[iter2 + 1]);
            SWAP_INT(&Basis_Sorted[iter2]   , &Basis_Sorted[iter2 + 1]);
            SWAP_INT(&Q_Number1_Sorted[iter2], &Q_Number1_Sorted[iter2 + 1]);
            SWAP_INT(&Q_Number2_Sorted[iter2], &Q_Number2_Sorted[iter2 + 1]);
   ...
   Dmrg_System->Q_Number2           = GET_ARRAY_INT1(dim_renorm);
   Dmrg_System->Q_Number3           = GET_ARRAY_INT1(dim_renorm);
   Dmrg_System->dim_renorm          = dim_renorm;
   Dmrg_System->dim_LLLR            = dim_LLLR;
   
   //Store Quantum Number
   for (iter1 = 0; iter1 < dim_renorm; iter1++) {
      Dmrg_System->Q_Number1[iter1] = Q_Number1_Sorted[iter1];
      Dmrg_System->Q_Number2[iter1] = Q_Number2_Sorted[iter1];
      Dmrg_System->Q_Number3[iter1] = Q_Number3_Sorted[iter1];
   }
   ...
   if (elem_num != Dmrg_System->Trans_Matrix->max_val) {
      printf("Error in DMRG_MAKE_TRANSFORMATION_MATRIX_Q3 at Check Point 3\n");
      exit(1);
   }
   ...
   FREE_ARRAY_INT1(Q_Number1_Sorted);
   FREE_ARRAY_INT1(Q_Number2_Sorted);
   FREE_ARRAY_INT1(Q_Number3_Sorted);
