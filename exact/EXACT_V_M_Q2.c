...
void EXACT_V_M_Q2(CRS1 *M_On, int qn1_out, int qn2_out, double *Vec, int qn1_in, int qn2_in, double *Out_Vec, char Sign_Flag[], int *N_Ele, int dim_onsite, int op_site, int p_threads, EXACT_WHOLE_BASIS_Q2 *W_Basis) {
   ...
   if (qn1_out < 0 || qn2_out < 0 || qn1_in < 0 || qn2_in < 0) {
      printf("Error in EXACT_V_M_Q2\n");
      printf("qn1_out=%d,qn2_out=%d,qn1_in=%d,qn2_in=%d\n", qn1_out, qn2_out, qn1_in, qn2_in);
      exit(1);
   }
   
   int dim_out   = W_Basis->Dim[qn1_out][qn2_out];
   int dim_in    = W_Basis->Dim[qn1_in][qn2_in];
...
#pragma omp parallel for private (whole_target_basis,local_basis,n_ele,sign,val,j,whole_a_basis,inv) num_threads (p_threads)
   for (i = 0; i < dim_out; i++) {
      whole_target_basis = W_Basis->Basis[qn1_out][qn2_out][i];
      ...
      for (j = M_On->Row[local_basis]; j < M_On->Row[local_basis + 1]; j++) {
         ...
         inv = BINARY_SEARCH_LINT1(W_Basis->Basis[qn1_in][qn2_in], 0, dim_in, whole_a_basis);
         if (inv >= 0) {
