...
DMRG_WHOLE_BASIS_Q2 *GET_WHOLE_BASIS_SUPERBLOCK(BLOCK *System, BLOCK *Enviro, MODEL_1DXXZ_VF *Model, DMRG_BASIS *Dmrg_Basis, DMRG_STATUS *Dmrg_Status) {
   ...
   int LL,LR,RR,RL,sz_sign,sz_map,tot_sz,count;
   int spin       = Model->spin;
   int max_sz     = spin*Model->tot_site;
   ...
   //Calculate Dim
#pragma omp parallel for private (LR,RR,RL,tot_sz,sz_sign,sz_map) num_threads (Model->p_threads)
...
#pragma omp parallel for private (LR,RR,RL,tot_sz,sz_sign,sz_map,count,inv_sup) num_threads (Model->p_threads)
   for (LL = 0; LL < dim_LL; LL++) {
      for (LR = 0; LR < dim_onsite; LR++) {
         for (RR = 0; RR < dim_RR; RR++) {
            for (RL = 0; RL < dim_onsite; RL++) {
