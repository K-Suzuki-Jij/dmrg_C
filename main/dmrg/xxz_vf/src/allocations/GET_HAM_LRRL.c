...
CRS1 *GET_HAM_LRRL(BLOCK *Block_System, BLOCK *Block_Enviro, DMRG_BASIS *Dmrg_Basis, DMRG_STATUS *Dmrg_Status, MODEL_1DXXZ_VF *Model) {
   ...
   int dim_LRRL = Dmrg_Basis->dim_LRRL;
   
   DMRG_A_BASIS **Dmrg_A_Basis = DMRG_GET_A_BASIS(dim_LRRL, Model->p_threads);
   HAM_BOX  **Box              = GET_HAM_BOX(Block_System, Block_Enviro, Model, LL_site, RR_site);
   
   long *Row_Elem_Num = GET_ARRAY_LINT1(dim_LRRL + 1);
   int num,inv,LR,RL,basis,thread_num;
   DMRG_BASIS_ONSITE Basis;
   
#pragma omp parallel for private (thread_num,Basis) num_threads (Model->p_threads)
   for (basis = 0; basis < dim_LRRL; basis++) {
      thread_num = omp_get_thread_num();
      Basis.LR   = Dmrg_Basis->LR_LRRL[basis];
      Basis.RL   = Dmrg_Basis->RL_LRRL[basis];
      MAKE_ELEMENT_HAM_LRRL(&Basis, Dmrg_A_Basis[thread_num], Dmrg_Basis->Inv_LRRL, Box[thread_num]);
...
#pragma omp parallel for reduction(+:tot_elem_num) num_threads (Model->p_threads)
   for (basis = 0; basis < dim_LRRL + 1; basis++) {
   ...
   CRS1 *Ham_LRRL = GET_CRS1(dim_LRRL, tot_elem_num);
   
#pragma omp parallel for private (thread_num,Basis,num,LR,RL,inv) num_threads (Model->p_threads)
   for (basis = 0; basis < dim_LRRL; basis++) {
      thread_num = omp_get_thread_num();
      Basis.LR   = Dmrg_Basis->LR_LRRL[basis];
      Basis.RL   = Dmrg_Basis->RL_LRRL[basis];
      ...
      for (num = 0; num < Dmrg_A_Basis[thread_num]->elem_num; num++) {
         ...
         inv = Dmrg_Basis->Inv_LRRL[LR][RL];
         if (inv >= 0) {
            Ham_LRRL->Col[Row_Elem_Num[basis]] = inv;
            Ham_LRRL->Val[Row_Elem_Num[basis]] = Dmrg_A_Basis[thread_num]->Val_LLLRRRRL[num];
            Row_Elem_Num[basis] = Row_Elem_Num[basis] + 1;
         }
         else {
            printf("Error in GET_HAM_LRRL\n");
            printf("inv = %d\n",inv);
            exit(1);
         }
      }
      Ham_LRRL->Row[basis + 1] = Dmrg_A_Basis[thread_num]->elem_num;
   }
      
   //Do not use openmp here
   for (basis = 0; basis < dim_LRRL; basis++) {
      Ham_LRRL->Row[basis + 1] = Ham_LRRL->Row[basis + 1] + Ham_LRRL->Row[basis];
   }
   
   //Check Point
   if (Ham_LRRL->Row[dim_LRRL] != tot_elem_num) {
      printf("Error in GET_HAM_LRRL\n");
      printf("%ld != %ld\n", Ham_LRRL->Row[dim_LRRL], tot_elem_num);
      exit(1);
   }
   ...
   SORT_COLUMN_CRS1(Ham_LRRL, Model->p_threads);
