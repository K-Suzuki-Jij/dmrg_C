...
void OUTPUT_SC_CORRELATIONS(SC_MAT_1DKLM_TVF *Sc_Mat, int site_start, int site_end, char File_Name[], char D1[], char D2[], MODEL_1DKLM_TVF *Model, DMRG_STATUS *Dmrg_Status) {
   ...
   fprintf(file_basis_row ,"###N=%d,Ne=%d,LocSpin=%1.1lf,P=%d,m=%d,BC=%s,Copy=%s,sweep=%d\n###t=%.1lf,J=%.4lf,I_xy=%.4lf,I_z=%.4lf,D_z=%.4lf,h_xc=%.4lf,h_xl=%.4lf,mu=%.4lf\n",
           Model->tot_site,
           Model->tot_ele,
   ...
   fprintf(file_basis_col ,"###N=%d,Ne=%d,LocSpin=%1.1lf,P=%d,m=%d,BC=%s,Copy=%s,sweep=%d\n###t=%.1lf,J=%.4lf,I_xy=%.4lf,I_z=%.4lf,D_z=%.4lf,h_xc=%.4lf,h_xl=%.4lf,mu=%.4lf\n",
           Model->tot_site,
           Model->tot_ele,
           (double)Model->spin_loc/2.0,
           Model->tot_parity,
   ...
   for (i = 0; i < out_num; i++) {
      ...
      fprintf(file_val ,"###N=%d,Ne=%d,LocSpin=%1.1lf,P=%d,m=%d,BC=%s,Copy=%s,sweep=%d\n###t=%.1lf,J=%.4lf,I_xy=%.4lf,I_z=%.4lf,D_z=%.4lf,h_xc=%.4lf,h_xl=%.4lf,mu=%.4lf\n",
              Model->tot_site,
              Model->tot_ele,
              (double)Model->spin_loc/2.0,
              Model->tot_parity,
   ...
   for (i = 0; i < out_num; i++) {
      ...
      for (r = 1; r < length; r++) {
         ...
         fprintf(file_vec ,"###N=%d,Ne=%d,LocSpin=%1.1lf,P=%d,m=%d,BC=%s,Copy=%s,sweep=%d\n###t=%.1lf,J=%.4lf,I_xy=%.4lf,I_z=%.4lf,D_z=%.4lf,h_xc=%.4lf,h_xl=%.4lf,mu=%.4lf\n",
                 Model->tot_site,
                 Model->tot_ele,
                 (double)Model->spin_loc/2.0,
                 Model->tot_parity,
