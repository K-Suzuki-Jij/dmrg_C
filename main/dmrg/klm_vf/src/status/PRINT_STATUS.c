...
void PRINT_STATUS(DMRG_STATUS *Dmrg_Status, DMRG_TIME *Dmrg_Time) {
   ...
   if (strcmp(Dmrg_Status->BC, "OBC") == 0 || strcmp(Dmrg_Status->BC, "SSD") == 0 || strcmp(Dmrg_Status->BC, "PBC_LL_LR_RL_RR") == 0) {
      printf("[%s]E/N=%.15lf(%.1e),N=%-3d(%-3d,1,1,%3d),dim=%-7d(%-4d,%-2d,%2d,%4d),Spin=%.1lf, Sz=%-4.1f, fill=%.3lf, T:%-6.1f(D:%-6.1f,II:%-6.1f,Ham:%-5.1f,Tr:%-4.1f),LL:%4.1lf%%,%3d/%3d, %2d/%d\n",
             Dmrg_Status->BC,
      ...
      fprintf(file, "[%s]E/N=%.15lf(%.1e),N=%-3d(%-3d,1,1,%3d),dim=%-7d(%-4d,%-2d,%2d,%4d),Spin=%.1lf, Sz=%-4.1f, fill=%.3lf, T:%-6.1f(D:%-6.1f,II:%-6.1f,Ham:%-5.1f,Tr:%-4.1f),LL:%4.1lf%%,%3d/%3d, %2d/%d\n",
              Dmrg_Status->BC,
              Dmrg_Status->gs_val/(Dmrg_Status->LL_site + Dmrg_Status->RR_site + 4),
              Dmrg_Status->gs_error,
              Dmrg_Status->LL_site + Dmrg_Status->RR_site + 4,
      ...
      printf("[%s]E/N=%.15lf(%.1e),N=%-3d(%-3d,1,%3d,1),dim=%-7d(%-4d,%-2d,%4d,%2d),Spin=%.1lf, Sz=%-4.1f, fill=%.3lf, T:%-6.1f(D:%-6.1f,II:%-6.1f,Ham:%-5.1f,Tr:%-4.1f),LL:%4.1lf,%3d/%3d, %2d/%d\n",
             Dmrg_Status->BC,
             Dmrg_Status->gs_val/(Dmrg_Status->LL_site + Dmrg_Status->RR_site + 4),
             Dmrg_Status->gs_error,
             Dmrg_Status->LL_site + Dmrg_Status->RR_site + 4,
      ...
      fprintf(file, "[%s]E/N=%.15lf(%.1e),N=%-3d(%-3d,1,%3d,1),dim=%-7d(%-4d,%-2d,%4d,%2d),Spin=%.1lf, Sz=%-4.1f, fill=%.3lf, T:%-6.1f(D:%-6.1f,II:%-6.1f,Ham:%-5.1f,Tr:%-4.1f),LL:%4.1lf,%3d/%3d, %2d/%d\n",
              Dmrg_Status->BC,
              Dmrg_Status->gs_val/(Dmrg_Status->LL_site + Dmrg_Status->RR_site + 4),
              Dmrg_Status->gs_error,
              Dmrg_Status->LL_site + Dmrg_Status->RR_site + 4,
