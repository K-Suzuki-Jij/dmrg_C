...
void EXPECTATION_SC_CORRELATIONS(BLOCK *System, BLOCK *Enviro, MODEL_1DHUBBARD_VF *Model, DMRG_PARAMETER *Dmrg_Param, double *Vec, DMRG_TIME *Dmrg_Time, DMRG_BASIS *Dmrg_Basis, DMRG_STATUS *Dmrg_Status) {
   ...
   SC_MAT_1DHUBBARD_VF     *Sc_Mat   = GET_SC_MAT_BASIS_1DHUBBARD_VF(Model);
   
   int del_sz,num,num1,num2;
   int max_del_sz = 2;
   int max_sz     = Model->tot_site + Model->tot_ele;
   int min_sz     = -Model->tot_site - Model->tot_ele;
   ...
   for (num1 = 0; num1 < dim_cc_onsite; num1++) {
      ONSITE_CC_HUBBARD(Sc_Mat->CC_Onsite[num1], 1.0);
   ...
   for (num1 = 0; num1 < dim_c_onsite; num1++) {
      ONSITE_C_HUBBARD(num1, Sc_Mat->C_Onsite[num1], 1.0);
   }
   ...
   for (del_sz = -max_del_sz; del_sz <= max_del_sz; del_sz++) {
      if (min_sz <= del_sz + Model->tot_sz && del_sz + Model->tot_sz <= max_sz) {
         SC_CORRELATIONS(del_sz, System, Enviro, Sc_Mat, Model, Dmrg_W_Basis, Dmrg_Status, Vec);
   ...
   FREE_SC_MAT_BASIS_1DHUBBARD_VF(Sc_Mat, Model);
