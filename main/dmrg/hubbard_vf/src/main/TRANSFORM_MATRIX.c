...
void TRANSFORM_MATRIX(BLOCK *Block_System, BLOCK *Block_Enviro, DMRG_BASIS *Dmrg_Basis, DMRG_SYSTEM_INFO *Dmrg_System, DMRG_STATUS *Dmrg_Status, MODEL_1DHUBBARD_VF *Model, DMRG_TIME *Dmrg_Time) {
   ...
   DMRG_EXTEND_AND_REDUCE(NULL, Block_System->NC_Up_On  , Block_System->NC_Up_RE[LL_site + 1]  , NULL                          , "LR", "No" , T_M, T_MD, M_CRS, M_CCS, V, Dmrg_Basis_LLLR);
   DMRG_EXTEND_AND_REDUCE(NULL, Block_System->NC_Down_On, Block_System->NC_Down_RE[LL_site + 1], NULL                          , "LR", "No" , T_M, T_MD, M_CRS, M_CCS, V, Dmrg_Basis_LLLR);
   ...
   if (strcmp(Model->BC, "PBC_LL_LR_RR_RL") == 0 || strcmp(Model->BC, "PBC_LL_LR_RL_RR") == 0) {
      ...
      DMRG_EXTEND_AND_REDUCE(Block_System->NC_Up_LE[LL_site]  , NULL, Block_System->NC_Up_LE[LL_site + 1]  , NULL                          , "LL", "No" , T_M, T_MD, M_CRS, M_CCS, V, Dmrg_Basis_LLLR);
      DMRG_EXTEND_AND_REDUCE(Block_System->NC_Down_LE[LL_site], NULL, Block_System->NC_Down_LE[LL_site + 1], NULL                          , "LL", "No" , T_M, T_MD, M_CRS, M_CCS, V, Dmrg_Basis_LLLR);
   }
