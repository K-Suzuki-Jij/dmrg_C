...
//  1DKLM_VF_EXACT
//
//  Created by Kohei Suzuki on 2019/07/11.
...
void MAKE_ELEMENT_HAM(long basis, EXACT_A_BASIS *A_Basis, HAM_BOX *Ham_Box, MODEL_1DKLM_VF *Model) {
   ...
   for(site = 0; site < tot_site - 1; site++){
      ...
      n_ele = ONSITE_FIND_SITE_ELE_SZBASIS_KLM(EXACT_FIND_SITE_STATE(basis, site, dim_onsite), Model->spin_loc);
      if (n_ele%2 == 1) {
         sign = 1;
      }
      else {
      ...
      EXACT_MAKE_ELEM_INTER(basis, site, site + 1, dim_onsite, Ham_Box->CUp_D_On  , Ham_Box->CUp_On    , &elem_num, Model->t*ssd_coeef, sign   , A_Basis);
      EXACT_MAKE_ELEM_INTER(basis, site, site + 1, dim_onsite, Ham_Box->CUp_On    , Ham_Box->CUp_D_On  , &elem_num, Model->t*ssd_coeef, -1*sign, A_Basis);
      EXACT_MAKE_ELEM_INTER(basis, site, site + 1, dim_onsite, Ham_Box->CDown_D_On, Ham_Box->CDown_On  , &elem_num, Model->t*ssd_coeef, sign   , A_Basis);
      EXACT_MAKE_ELEM_INTER(basis, site, site + 1, dim_onsite, Ham_Box->CDown_On  , Ham_Box->CDown_D_On, &elem_num, Model->t*ssd_coeef, -1*sign, A_Basis);
   }
   ...
   if (strcmp(Model->BC, "PBC") == 0) {
      ...
      n_ele = 0;
      for(site = 0; site < tot_site - 1; site++){
         n_ele = n_ele + ONSITE_FIND_SITE_ELE_SZBASIS_KLM(EXACT_FIND_SITE_STATE(basis, site, dim_onsite), Model->spin_loc);
      }
      if (n_ele%2 == 1) {
         sign = 1;
      }
      ...
      EXACT_MAKE_ELEM_INTER(basis, 0, Model->tot_site - 1, dim_onsite, Ham_Box->CUp_D_On  , Ham_Box->CUp_On    , &elem_num, Model->t, sign   , A_Basis);
      EXACT_MAKE_ELEM_INTER(basis, 0, Model->tot_site - 1, dim_onsite, Ham_Box->CUp_On    , Ham_Box->CUp_D_On  , &elem_num, Model->t, -1*sign, A_Basis);
      EXACT_MAKE_ELEM_INTER(basis, 0, Model->tot_site - 1, dim_onsite, Ham_Box->CDown_D_On, Ham_Box->CDown_On  , &elem_num, Model->t, sign   , A_Basis);
      EXACT_MAKE_ELEM_INTER(basis, 0, Model->tot_site - 1, dim_onsite, Ham_Box->CDown_On  , Ham_Box->CDown_D_On, &elem_num, Model->t, -1*sign, A_Basis);
   }
