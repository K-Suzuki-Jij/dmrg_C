...
void SC_CORRELATIONS(int sc_sz, int site_start, double *GS_Vec, SC_MAT_1DKLM_VF *Sc_Mat, EXACT_WHOLE_BASIS_Q3 *W_Basis, MODEL_1DKLM_VF *Model) {
   ...
   MAKE_SC_MAT_BASIS_1DKLM_VF(sc_sz, Sc_Mat, Model);
   ...
   int tot_sz_map         = (1 - SIGN(tot_sz))/2;
   ...
   for (i = 0; i < Model->dim_onsite; i++) {
      N_Ele[i] = ONSITE_FIND_SITE_ELE_SZBASIS_KLM(i, Model->spin_loc);
   }
...
#pragma omp parallel for private (ind1,sz1,sz_map1) num_threads (Model->p_threads)
   for (i = 0; i < Sc_Mat->dim_ccsl_1; i++) {
      ...
      EXACT_V_M_Q3(Ham_Box->CCSL_On[ind1], 2, sz_map1, abs(sz1), GS_Vec, 0, tot_sz_map, abs(tot_sz), Vec_CCSL_1[i], "No", N_Ele, dim_onsite, site_start - 1, 1, W_Basis);
   }
...
#pragma omp parallel for private (ind1,sz1,sz_map1) num_threads (Model->p_threads)
   for (i = 0; i < Sc_Mat->dim_ccsl_2; i++) {
      ...
      sz1     = Sc_Mat->CCSL_Sz[ind1] + tot_sz;
      sz_map1 = (1 - SIGN(sz1))/2;
      EXACT_V_M_Q3(Ham_Box->CCSL_On[ind1], 2, sz_map1, abs(sz1), GS_Vec, 0, tot_sz_map, abs(tot_sz), Vec_CCSL_2[i], "No", N_Ele, dim_onsite, site_start, 1, W_Basis);
   }
...
#pragma omp parallel for private (sz1,sz_map1) num_threads (Model->p_threads)
   for (i = 0; i < Model->dim_csl_onsite; i++) {
      sz1 = Sc_Mat->CSL_Sz[i] + tot_sz;
      sz_map1 = (1 - SIGN(sz1))/2;
      EXACT_V_M_Q3(Ham_Box->CSL_On[i], 1, sz_map1, abs(sz1), GS_Vec, 0, tot_sz_map, abs(tot_sz), Vec_CSL[i], "Yes", N_Ele, dim_onsite, site_start - 1, 1, W_Basis);
   }
...
#pragma omp parallel for private (ind1,ind2,sz1,sz2,sz_map1,sz_map2) num_threads (Model->p_threads)
   for (i = 0; i < Sc_Mat->dim_csl_csl; i++) {
      ...
      sz1     = Sc_Mat->CSL_Sz[ind1];
      sz2     = Sc_Mat->CSL_Sz[ind2];
      sz_map1 = (1 - SIGN(sz2 + tot_sz))/2;
      sz_map2 = (1 - SIGN(sz2 + sz1 + tot_sz))/2;
      EXACT_V_M_Q3(Ham_Box->CSL_On[ind1], 2, sz_map2, abs(sz2 + sz1 + tot_sz), Vec_CSL[ind2], 1, sz_map1, abs(sz2 + tot_sz), Vec_CSL_CSL[i], "Yes", N_Ele, dim_onsite, site_start, 1, W_Basis);
   }
...
#pragma omp parallel for private (thread_num,ind1,sz1,site,r,j,jj,ii,ind2,sz2,sz_map1,sz_map2) num_threads (Model->p_threads)
   for (i = 0; i < Sc_Mat->dim_tot; i++) {
      ...
      if (i < Sc_Mat->dim_ccsl_1) {
         ind1 = Sc_Mat->CCSL_1_Num[i];
         sz1  = Sc_Mat->CCSL_Sz[ind1];
         sz_map1 = (1 - SIGN(sz1 + tot_sz))/2;
         for (site = site_start; site < tot_site - 1; site++) {
            r = site - site_start;
            EXACT_V_M_Q3(Ham_Box->CCSL_On[ind1], 2, sz_map1, abs(sz1 + tot_sz), GS_Vec, 0, tot_sz_map, abs(tot_sz), Vec_Temp_1[thread_num], "No", N_Ele, dim_onsite, site, 1, W_Basis);
            
            for (j = 0; j < Sc_Mat->dim_tot; j++) {
               
               //Reference: Onsite_2
               if (j < Sc_Mat->dim_ccsl_1) {
                  Sc_Mat->Mat[r][j][i] = INNER_PRODUCT(Vec_CCSL_2[j], Vec_Temp_1[thread_num], W_Basis->Dim[2][sz_map1][abs(sz1 + tot_sz)], 1);
               }
               //Reference: Intersite
               else if (Sc_Mat->dim_ccsl_1 <= j && j < Sc_Mat->dim_ccsl_1 + Sc_Mat->dim_csl_csl) {
                  jj = j - Sc_Mat->dim_ccsl_1;
                  Sc_Mat->Mat[r][j][i] = INNER_PRODUCT(Vec_CSL_CSL[jj], Vec_Temp_1[thread_num], W_Basis->Dim[2][sz_map1][abs(sz1 + tot_sz)], 1);
               }
               
               //Reference: Onsite_1
               else {
                  jj = j - (Sc_Mat->dim_ccsl_1 + Sc_Mat->dim_csl_csl);
                  Sc_Mat->Mat[r][j][i] = INNER_PRODUCT(Vec_CCSL_1[jj], Vec_Temp_1[thread_num], W_Basis->Dim[2][sz_map1][abs(sz1 + tot_sz)], 1);
               }
      ...
      //Length: Intersite
         ...
         for (site = site_start; site < tot_site - 1; site++) {
            r = site - site_start;
            EXACT_V_M_Q3(Ham_Box->CSL_On[ind2], 1, sz_map1, abs(sz2 + tot_sz)      , GS_Vec                , 0, tot_sz_map, abs(tot_sz)      , Vec_Temp_1[thread_num], "Yes", N_Ele, dim_onsite, site + 1, 1, W_Basis);
            EXACT_V_M_Q3(Ham_Box->CSL_On[ind1], 2, sz_map2, abs(sz1 + sz2 + tot_sz), Vec_Temp_1[thread_num], 1, sz_map1   , abs(sz2 + tot_sz), Vec_Temp_2[thread_num], "Yes", N_Ele, dim_onsite, site    , 1, W_Basis);
            
            for (j = 0; j < Sc_Mat->dim_tot; j++) {
               
               //Reference: Onsite_2
               if (j < Sc_Mat->dim_ccsl_1) {
                  Sc_Mat->Mat[r][j][i] = INNER_PRODUCT(Vec_CCSL_2[j], Vec_Temp_2[thread_num], W_Basis->Dim[2][sz_map2][abs(sz1 + sz2 + tot_sz)], 1);
               }
               //Reference: Intersite
               else if (Sc_Mat->dim_ccsl_1 <= j && j < Sc_Mat->dim_ccsl_1 + Sc_Mat->dim_csl_csl) {
                  jj = j - Sc_Mat->dim_ccsl_1;
                  Sc_Mat->Mat[r][j][i] = INNER_PRODUCT(Vec_CSL_CSL[jj], Vec_Temp_2[thread_num], W_Basis->Dim[2][sz_map2][abs(sz1 + sz2 + tot_sz)], 1);
               }
               
               //Reference: Onsite_1
               else {
                  jj = j - (Sc_Mat->dim_ccsl_1 + Sc_Mat->dim_csl_csl);
                  Sc_Mat->Mat[r][j][i] = INNER_PRODUCT(Vec_CCSL_1[jj], Vec_Temp_2[thread_num], W_Basis->Dim[2][sz_map2][abs(sz1 + sz2 + tot_sz)], 1);
               }
      ...
      //Length: Onsite_2
         ...
         for (site = site_start; site < tot_site - 1; site++) {
            r = site - site_start;
            EXACT_V_M_Q3(Ham_Box->CCSL_On[ind1], 2, sz_map1, abs(sz1 + tot_sz), GS_Vec, 0, tot_sz_map, abs(tot_sz), Vec_Temp_1[thread_num], "No", N_Ele, dim_onsite, site + 1, 1, W_Basis);
