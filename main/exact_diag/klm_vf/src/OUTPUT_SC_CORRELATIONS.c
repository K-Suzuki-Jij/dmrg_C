...
void OUTPUT_SC_CORRELATIONS(SC_MAT_1DKLM_VF *Sc_Mat, int site_start, int site_end, char File_Name[], char D1[], char D2[], MODEL_1DKLM_VF *Model) {
   ...
   fprintf(file_basis_row ,"###N=%d,Ne=%d,LocSpin=%1.1lf,Sz=%1.1lf,BC=%s\n###t=%.1lf,J=%.3lf,I_xy=%.3lf,I_z=%.3lf,D_z=%.3lf,h_z=%.3lf,mu=%.5lf\n",
           Model->tot_site,
           Model->tot_ele,
   ...
   fprintf(file_basis_col ,"###N=%d,Ne=%d,LocSpin=%1.1lf,Sz=%1.1lf,BC=%s\n###t=%.1lf,J=%.3lf,I_xy=%.3lf,I_z=%.3lf,D_z=%.3lf,h_z=%.3lf,mu=%.5lf\n",
           Model->tot_site,
           Model->tot_ele,
           (double)Model->spin_loc/2.0,
           (double)Model->tot_sz/2,
   ...
   for (i = 0; i < out_num; i++) {
      ...
      fprintf(file_val,"###N=%d,Ne=%d,LocSpin=%1.1lf,Sz=%1.1lf,BC=%s\n###t=%.1lf,J=%.3lf,I_xy=%.3lf,I_z=%.3lf,D_z=%.3lf,h_z=%.3lf,mu=%.5lf\n",
              Model->tot_site,
              Model->tot_ele,
              (double)Model->spin_loc/2.0,
              (double)Model->tot_sz/2,
   ...
   for (i = 0; i < out_num; i++) {
      ...
      for (r = 0; r < length; r++) {
         ...
         fprintf(file_vec,"###N=%d,Ne=%d,LocSpin=%1.1lf,Sz=%1.1lf,BC=%s\n###t=%.1lf,J=%.3lf,I_xy=%.3lf,I_z=%.3lf,D_z=%.3lf,h_z=%.3lf,mu=%.5lf\n",
                 Model->tot_site,
                 Model->tot_ele,
                 (double)Model->spin_loc/2.0,
                 (double)Model->tot_sz/2,
